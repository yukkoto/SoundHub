(() => {
  ...
  function buildQueue() {
    const seen = new Set();
    queue = [...document.querySelectorAll('[data-play]')]
      .map(el => {
        try {
          return JSON.parse(el.getAttribute('data-play'));
        } catch (_) {
          return null;
        }
      })
    ...
    currentIndex = currentTrack?.id ? queue.findIndex(track => track.id === currentTrack.id) : -1;
    updateSkipButtons();
  }
  ...
  function highlightCurrentTrack() {
    ...
    document.querySelectorAll('[data-play]').forEach(el => {
      const raw = el.getAttribute('data-play');
      if (!raw || !raw.includes(`"id":"${currentTrack.id}"`)) return;
      const card = el.closest('.track-unit, .wall-card, .hero-track, .feature-cover, .feature-panel');
  ...
  function updateSkipButtons() {
    ...
    const hasPrev = hasQueue && currentIndex > 0;
    const hasNext = hasQueue && currentIndex >= 0 && currentIndex < queue.length - 1;

    [prevBtn, fullPrevBtn].forEach(btn => {
      if (!btn) return;
      btn.disabled = !hasPrev;
      btn.setAttribute('aria-disabled', hasPrev ? 'false' : 'true');
    });

    [nextBtn, fullNextBtn].forEach(btn => {
      if (!btn) return;
      btn.disabled = !hasNext;
      btn.setAttribute('aria-disabled', hasNext ? 'false' : 'true');
    });
  }

  function updatePlayCounters(trackId, plays) {
    if (!trackId || !Number.isFinite(Number(plays))) return;
    document
      .querySelectorAll(`[data-track-plays-value="${CSS.escape(trackId)}"]`)
      .forEach(node => {
        node.textContent = String(plays);
      });

    if (currentTrack?.id === trackId) {
      currentTrack.plays = Number(plays);
    }
  }

  async function reportPlay(trackId) {
    try {
      const res = await fetch(`/api/play/${encodeURIComponent(trackId)}`, { method: 'POST' });
      const json = await res.json();
      if (!json?.ok) return;
      updatePlayCounters(trackId, json.plays);
    } catch (_) {}
  }
  ...
  function playRelative(offset) {
    ...
    if (currentIndex < 0 && currentTrack?.id) {
      currentIndex = queue.findIndex(track => track.id === currentTrack.id);
    }
    const targetIndex = currentIndex + offset;
    if (targetIndex < 0 || targetIndex >= queue.length) return;
    playTrack(queue[targetIndex]);
  }
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'ArrowLeft' && (e.altKey || e.metaKey)) playRelative(-1);
    if (e.key === 'ArrowRight' && (e.altKey || e.metaKey)) playRelative(1);
  });
