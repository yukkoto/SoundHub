...
function deleteManagedUpload(publicUrl) {
  ...
  const absPath = path.join(__dirname, 'public', publicUrl.replace(/^\//, ''));
  try {
...
app.post('/admin/tracks/:id/delete', requireRole('admin'), (req, res) => {
  const id = req.params.id;
  const tracks = readTracksRaw().map(normalizeTrack);
  const removed = tracks.find(x => x.id === id);
...
app.post('/api/play/:trackId', (req, res) => {
  const trackId = req.params.trackId;
  const user = getCurrentUser(req);
  ...
  return res.json({ ok: true, trackId: track.id, plays: track.plays });
});
